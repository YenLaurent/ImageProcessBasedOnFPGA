...
//!4. 实际的vsync信号与本模块vsync逻辑是相反的，即场同步信号在发送完一帧图像后有效，在例化时一定要特别注意将摄像头模块输出vsync信号取反
//*5. 由于加入了2字节行号，加之一段式状态机本身的单拍延时特性，写入FIFO的数据将会延迟3拍完成，
//*   也就是说，摄像头的输入像素数据在每行之间至少间隔3拍时钟周期，才能保证数据的正确性，
...
//    而写入行号时则是连续写入的
//*7. 该模块认为场同步信号在每帧图像传输完成后拉低（无效）
//*   行同步信号在每行传输完成后拉低（无效）
//!8. 该模块认为输入数据有效信号与hsync信号是同步的，即在行同步信号有效时，数据有效信号也是有效的，这或许需要后续对摄像头模块、图像预处理模块的信号进行调整
// 9. 首先发送行号的第八位，而后发送行号的高八位
//////////////////////////////////////////////////////////////////////////////////
